...
class ConcertInfo(metaclass=Metaclass_ConcertInfo):
    ...
    _fields_and_field_types = {
        ...
        'prices': 'sequence<int64>',
        'remng_durations': 'sequence<int64>',
        'remng_ticket': 'sequence<int64>',
    }
    ...
    def __init__(self, **kwargs):
        ...
        self.prices = array.array('q', kwargs.get('prices', []))
        self.remng_durations = array.array('q', kwargs.get('remng_durations', []))
        self.remng_ticket = array.array('q', kwargs.get('remng_ticket', []))
    ...
    def __eq__(self, other):
        ...
        if self.remng_durations != other.remng_durations:
            return False
        if self.remng_ticket != other.remng_ticket:
            return False
        return True
    ...
    def concert_count(self, value):
        if self._check_fields:
            ...
            assert value >= -9223372036854775808 and value < 9223372036854775808, \
                "The 'concert_count' field must be an integer in [-9223372036854775808, 9223372036854775807]"
        ...
        if self._check_fields:
            if isinstance(value, array.array):
                assert value.typecode == 'q', \
                    "The 'prices' array.array() must have the type code of 'q'"
                self._prices = value
                return
            from collections.abc import Sequence
            from collections.abc import Set
            ...
            assert \
                ...
                "The 'prices' field must be a set or sequence and each value of type 'int' and each integer in [-9223372036854775808, 9223372036854775807]"
        self._prices = array.array('q', value)
    ...
    def remng_durations(self, value):
        if self._check_fields:
            if isinstance(value, array.array):
                assert value.typecode == 'q', \
                    "The 'remng_durations' array.array() must have the type code of 'q'"
            ...
            assert \
                ((isinstance(value, Sequence) or
                 ...
                 all(isinstance(v, int) for v in value) and
                 all(val >= -9223372036854775808 and val < 9223372036854775808 for val in value)), \
                "The 'remng_durations' field must be a set or sequence and each value of type 'int' and each integer in [-9223372036854775808, 9223372036854775807]"
        self._remng_durations = array.array('q', value)

    @builtins.property
    def remng_ticket(self):
        """Message field 'remng_ticket'."""
        return self._remng_ticket

    @remng_ticket.setter
    def remng_ticket(self, value):
        if self._check_fields:
            if isinstance(value, array.array):
                assert value.typecode == 'q', \
                    "The 'remng_ticket' array.array() must have the type code of 'q'"
                self._remng_ticket = value
                return
            from collections.abc import Sequence
            from collections.abc import Set
            from collections import UserList
            ...
            assert \
                ((isinstance(value, Sequence) or
                 ...
                 all(isinstance(v, int) for v in value) and
                 all(val >= -9223372036854775808 and val < 9223372036854775808 for val in value)), \
                "The 'remng_ticket' field must be a set or sequence and each value of type 'int' and each integer in [-9223372036854775808, 9223372036854775807]"
        self._remng_ticket = array.array('q', value)
