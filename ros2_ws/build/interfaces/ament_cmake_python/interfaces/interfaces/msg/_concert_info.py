...
class ConcertInfo(metaclass=Metaclass_ConcertInfo):
    ...
    _fields_and_field_types = {
        'concert_count': 'uint32',
        'concert_names': 'sequence<string>',
        'musn_names': 'sequence<string>',
        'concert_dates': 'sequence<string>',
        'prices': 'sequence<uint32>',
        'remng_durations': 'sequence<uint32>',
        'remng_ticket': 'sequence<uint32>',
    }
    ...
    def __init__(self, **kwargs):
        ...
        self.prices = array.array('I', kwargs.get('prices', []))
        self.remng_durations = array.array('I', kwargs.get('remng_durations', []))
        self.remng_ticket = array.array('I', kwargs.get('remng_ticket', []))
    ...
    def concert_count(self, value):
        if self._check_fields:
            ...
            assert value >= 0 and value < 4294967296, \
                "The 'concert_count' field must be an unsigned integer in [0, 4294967295]"
        ...
        if self._check_fields:
            if isinstance(value, array.array):
                assert value.typecode == 'I', \
                    "The 'prices' array.array() must have the type code of 'I'"
                self._prices = value
                return
            from collections.abc import Sequence
            from collections.abc import Set
            ...
            assert \
                ...
                "The 'prices' field must be a set or sequence and each value of type 'int' and each unsigned integer in [0, 4294967295]"
        self._prices = array.array('I', value)
    ...
    def remng_durations(self, value):
        if self._check_fields:
            if isinstance(value, array.array):
                assert value.typecode == 'I', \
                    "The 'remng_durations' array.array() must have the type code of 'I'"
            ...
            assert \
                ((isinstance(value, Sequence) or
                 ...
                 all(isinstance(v, int) for v in value) and
                 all(val >= 0 and val < 4294967296 for val in value)), \
                "The 'remng_durations' field must be a set or sequence and each value of type 'int' and each unsigned integer in [0, 4294967295]"
        self._remng_durations = array.array('I', value)
    ...
    def remng_ticket(self, value):
        if self._check_fields:
            if isinstance(value, array.array):
                assert value.typecode == 'I', \
                    "The 'remng_ticket' array.array() must have the type code of 'I'"
            ...
            assert \
                ((isinstance(value, Sequence) or
                 ...
                 all(isinstance(v, int) for v in value) and
                 all(val >= 0 and val < 4294967296 for val in value)), \
                "The 'remng_ticket' field must be a set or sequence and each value of type 'int' and each unsigned integer in [0, 4294967295]"
        self._remng_ticket = array.array('I', value)
